...
async def test_turn_endpoint_light(monkeypatch):
    ...
    # v2 Architecture (2026-01-28): Only 2 essential workers per-turn
    # Other workers (planner, insight, ayurvedic, etc.) moved to daily schedule
    assert "turn_memory_update" in jobs, f"turn_memory_update not in {jobs}"
    assert "episodic_consolidation_v21" in jobs, f"episodic_consolidation_v21 not in {jobs}"
