...
from sakhi.apps.engine.micro_momentum.engine import generate_micro_momentum
...
@pytest.mark.asyncio
async def test_micro_momentum_engine(monkeypatch):
    async def fake_resolve(pid):
        return pid

    async def fake_q(sql, *args, **kwargs):
        if "morning_preview_cache" in sql:
            return {"key_tasks": ["a"], "focus_areas": ["focus"], "reminders": ["r1"]}
        if "morning_momentum_cache" in sql:
            return {"momentum_hint": "hint", "suggested_start": "start", "reason": "r"}
        if "morning_ask_cache" in sql:
            return {"question": "q", "reason": "ask"}
        if "daily_closure_cache" in sql:
            return {"pending": ["p1"]}
        if "tasks" in sql:
            return {"labels": ["t1", "t2", "t3", "t4"]}
        return {}

    monkeypatch.setattr("sakhi.apps.engine.micro_momentum.engine.resolve_person_id", fake_resolve)
    monkeypatch.setattr("sakhi.apps.engine.micro_momentum.engine.q", fake_q)

    res = await generate_micro_momentum("p1")
    assert res["nudge"]
    assert res["reason"]


@pytest.mark.asyncio
async def test_micro_momentum_deterministic(monkeypatch):
    async def fake_resolve(pid):
        return pid
    ...
    a1 = await generate_micro_momentum("p1")
    a2 = await generate_micro_momentum("p1")
    assert a1["nudge"] == a2["nudge"]
    assert a1["reason"] == a2["reason"]


@pytest.mark.asyncio
async def test_micro_momentum_worker(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr("sakhi.apps.engine.micro_momentum.engine.resolve_person_id", fake_resolve)
    monkeypatch.setattr("sakhi.apps.engine.micro_momentum.engine.q", fake_q)
    monkeypatch.setattr("sakhi.apps.engine.micro_momentum.engine.dbexec", fake_exec)

    await run_micro_momentum("p1")
    assert any("micro_momentum_cache" in c for c in calls)
    assert any("personal_model" in c for c in calls)


def test_micro_momentum_scheduler(monkeypatch):
    enqueued = []
    ...
    scheduler.schedule_micro_momentum()
    assert "run_micro_momentum" in enqueued


@pytest.mark.asyncio
async def test_micro_momentum_route(monkeypatch):
    async def fake_q(sql, *args, **kwargs):
        return [
            {
                "nudge_date": datetime.date.today(),
                "nudge": "n1",
                "reason": "r",
                "generated_at": "now",
            }
    ...
    monkeypatch.setattr("sakhi.apps.api.routes.micro_momentum.q", fake_q)
    monkeypatch.setattr("sakhi.apps.api.routes.micro_momentum.resolve_person_id", fake_resolve)
    app.dependency_overrides[auth_deps.get_current_user_id] = lambda: "user"

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        resp = await ac.get("/v1/micro_momentum", params={"person_id": "p1"})
    assert resp.status_code == 200
    assert resp.json()["nudge"] == "n1"


@pytest.mark.asyncio
async def test_micro_momentum_in_turn(monkeypatch):
    class FakeDateTime(datetime.datetime):
        @classmethod
        def utcnow(cls):
            return datetime.datetime(2025, 1, 1, 14, 0, 0)
    ...
    async def fake_q(sql, *args, **kwargs):
        if "micro_momentum_cache" in sql:
            return [
                {
                    "nudge_date": datetime.date(2025, 1, 1),
                    "nudge": "n1",
                    "reason": "r",
                    "generated_at": "now",
                }
            ]
        if "morning_momentum_cache" in sql or "morning_ask_cache" in sql or "morning_preview_cache" in sql or "daily_reflection_cache" in sql or "daily_closure_cache" in sql:
            return []
        return []
