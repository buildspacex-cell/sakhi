...
function PersonaDetailsClient() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/persona-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Persona Worker Details (Lab)</h1>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Card title="Personal model (persona/identity/soul)">
            ...
            <Card title="Persona snapshot (context cache)">
              <pre style={{ background: "#0b1220", border: `1px solid ${palette.border}`, borderRadius: 10, padding: 10, fontSize: 12, color: palette.text, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                {JSON.stringify({ updated_at: data.cache_updated_at, persona_snapshot: data.persona_snapshot }, null, 2)}
              </pre>
            </Card>
          </div>
        )}
