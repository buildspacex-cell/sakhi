...
type RhythmDetails = {
  ...
  state: any;
  daily_curves: any[];
  planner_alignment: any[];
};

function RhythmDetailsClient() {
  "use client";

  const params = useSearchParams();
  const [data, setData] = useState<RhythmDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/rhythm-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Rhythm Worker Details (Lab)</h1>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <Card title="Daily curves">
              {data.daily_curves?.length ? (
                <ul style={{ paddingLeft: 16, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                  {data.daily_curves.map((row: any) => (
                    <li key={`${row.day_scope}-${row.created_at}`} style={{ color: palette.text }}>
                      <div style={{ fontSize: 12, color: palette.muted }}>{row.created_at}</div>
                      <div>Day: {row.day_scope}</div>
            ...
            <Card title="Planner alignment">
              {data.planner_alignment?.length ? (
                <ul style={{ paddingLeft: 16, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                  {data.planner_alignment.map((row: any) => (
                    <li key={`${row.horizon}-${row.generated_at}`} style={{ color: palette.text }}>
                      <div style={{ fontSize: 12, color: palette.muted }}>{row.generated_at}</div>
                      <div>Horizon: {row.horizon}</div>
                      <div style={{ fontSize: 12, color: palette.muted }}>{JSON.stringify(row.recommendations)}</div>
                    </li>
                  ))}
                </ul>
              ) : (
...
export default function RhythmDetailsPage() {
  return <RhythmDetailsClient />;
}
