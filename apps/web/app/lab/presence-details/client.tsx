...
function PresenceDetailsClient() {
  "use client";

  const params = useSearchParams();
  const [data, setData] = useState<PresenceDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/presence-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Presence / Follow-Ups Details (Lab)</h1>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Card title="Presence state">
              {data.presence_state?.length ? (
                <ul style={{ paddingLeft: 16, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                  {data.presence_state.map((row: any, idx: number) => (
                    <li key={idx} style={{ color: palette.text }}>
                      <div style={{ fontSize: 12, color: palette.muted }}>{row.created_at}</div>
                      <div>Date: {row.date}</div>
                      <div style={{ fontSize: 12, color: palette.muted }}>Mood: {row.mood_today} • Open actions: {JSON.stringify(row.open_actions)}</div>
                      <div style={{ fontSize: 12, color: palette.muted }}>{row.summary}</div>
                    </li>
                  ))}
                </ul>
              ) : (
            ...
            <Card title="Insights (presence/follow-ups)">
...
export default function PresenceDetailsPage() {
  return <PresenceDetailsClient />;
}
