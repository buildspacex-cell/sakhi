...
function MaintenanceTempoDetailsClient() {
  "use client";

  const params = useSearchParams();
  const [data, setData] = useState<MaintenanceDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/maintenance-tempo-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Maintenance / Tempo Details (Lab)</h1>
...
export default function MaintenanceTempoDetailsPage() {
  return <MaintenanceTempoDetailsClient />;
}
