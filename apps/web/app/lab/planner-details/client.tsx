...
function PlannerDetailsClient() {
  "use client";

  const params = useSearchParams();
  const [data, setData] = useState<PlannerDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/planner-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Planner Worker Details (Lab)</h1>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Card title="Recent planned items">
              {data.tasks?.length ? (
                <ul style={{ paddingLeft: 16, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                  {data.tasks.map((t: any) => (
                    <li key={t.id} style={{ color: palette.text }}>
                      <div style={{ fontSize: 12, color: palette.muted }}>{t.created_at}</div>
                      <div>{t.label}</div>
                      <div style={{ fontSize: 12, color: palette.muted }}>
                        status: {t.status} • horizon: {t.horizon} • priority: {t.priority} {t.due_ts ? `• due: ${t.due_ts}` : ""}
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <Card title="Task window (context cache)">
              <pre style={{ background: "#0b1220", border: `1px solid ${palette.border}`, borderRadius: 10, padding: 10, fontSize: 12, color: palette.text, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                {JSON.stringify({ updated_at: data.cache_updated_at, task_window: data.task_window }, null, 2)}
              </pre>
            </Card>
          </div>
        )}
...
export default function PlannerDetailsPage() {
  return <PlannerDetailsClient />;
}
