...
type MetaReflectionDetails = {
  person_id: string;
  scores: any;
  queue: any[];
  continuity: any;
  meta_reflections: any[];
};

function MetaReflectionDetailsClient() {
  "use client";

  const params = useSearchParams();
  const [data, setData] = useState<MetaReflectionDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const url = new URL("/api/lab/meta-reflection-details", window.location.origin);
  ...
  return (
    <div style={{ minHeight: "100vh", background: palette.bg, color: palette.text, padding: 20, fontFamily: "Inter, system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>Meta-Reflection Worker Details (Lab)</h1>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <Card title="Insights queue (meta_reflection)">
              {data.queue?.length ? (
            ...
            <Card title="Meta reflections">
              {data.meta_reflections?.length ? (
                <ul style={{ paddingLeft: 16, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                  {data.meta_reflections.map((row: any) => (
...
export default function MetaReflectionDetailsPage() {
  return <MetaReflectionDetailsClient />;
}
