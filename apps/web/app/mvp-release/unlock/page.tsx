...
async function unlockPresentation(formData: FormData) {
  "use server";

  const password = String(formData.get("password") || "").trim();

  if (!matchesMvpPresentationPassword(password)) {
    redirect("/mvp-release/unlock?error=1");
  }
  ...
  redirect("/mvp-release");
}

export default async function MvpReleaseUnlockPage({
  searchParams,
}: {
  searchParams?: {
    error?: string;
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-3xl items-center justify-center px-6 py-16">
        <div className="w-full max-w-md rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,22,38,0.96),rgba(7,12,22,0.92))] p-10 shadow-[0_40px_120px_rgba(0,0,0,0.38)]">
          ...
          <p className="mt-4 text-base leading-7 text-slate-300">
            Enter the shared password to open the Sakhi MVP deck.
          </p>
          ...
          <form action={unlockPresentation} className="mt-8 space-y-4">
