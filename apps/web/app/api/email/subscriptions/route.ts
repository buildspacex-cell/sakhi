 ...
 *
 * Returns active subscription/renewal trackers.
 */
export async function GET() {
  try {
    ...
    const backendResponse = await fetch(
      `${apiBase}/email/subscriptions?person_id=${personId}`,
      {
        method: "GET",
    ...
    console.error("Error in GET /api/email/subscriptions:", error);
    return NextResponse.json({ subscriptions: [], count: 0 }, { status: 500 });
  }
}
