...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(
      `${getApiBase()}/api/v1/agent/recurring/${encodeURIComponent(scheduleId)}/cancel?person_id=${encodeURIComponent(personId)}`,
      {
        method: "POST",
    ...
    console.error("[api/agent/recurring/:scheduleId/cancel] proxy error:", error);
    return NextResponse.json(
