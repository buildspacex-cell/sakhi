...
@pytest.mark.asyncio
async def test_mini_flow_engine(monkeypatch):
    async def fake_resolve(pid):
        return pid

    async def fake_q(sql, *args, **kwargs):
        if "focus_path_cache" in sql:
            return {"progress_step": "step", "closure_step": "close", "intent_source": "focus_path"}
        if "tasks" in sql:
            return {"labels": ["t1", "t2"]}
        return {}
    ...
    res = await generate_mini_flow("p1")
    assert res["warmup_step"]
    assert res["focus_block_step"]
    assert res["closure_step"]


@pytest.mark.asyncio
async def test_mini_flow_deterministic(monkeypatch):
    async def fake_resolve(pid):
        return pid
    ...
    a1 = await generate_mini_flow("p1")
    a2 = await generate_mini_flow("p1")
    assert a1["warmup_step"] == a2["warmup_step"]
    assert a1["focus_block_step"] == a2["focus_block_step"]


@pytest.mark.asyncio
async def test_mini_flow_worker(monkeypatch):
    calls = []
...
def test_mini_flow_scheduler(monkeypatch):
    enqueued = []
    ...
    scheduler.schedule_mini_flow()
    assert "run_mini_flow" in enqueued


@pytest.mark.asyncio
async def test_mini_flow_route(monkeypatch):
    async def fake_resolve(pid):
        return pid

    async def fake_generate(pid, focus_path=None):
        return {
    ...
    async def fake_persist(pid, flow):
        return None

    monkeypatch.setattr("sakhi.apps.api.routes.mini_flow.resolve_person_id", fake_resolve)
    monkeypatch.setattr("sakhi.apps.api.routes.mini_flow.generate_mini_flow", fake_generate)
    monkeypatch.setattr("sakhi.apps.api.routes.mini_flow.persist_mini_flow", fake_persist)
    app.dependency_overrides[auth_deps.get_current_user_id] = lambda: "user"

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        resp = await ac.post("/v1/mini_flow", json={"person_id": "p1"})
    assert resp.status_code == 200
    assert resp.json()["focus_block_step"] == "f"


@pytest.mark.asyncio
async def test_mini_flow_in_turn(monkeypatch):
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        resp = await ac.post("/v2/turn", json={"text": "give me a short routine"})
    assert resp.status_code == 200
    data = resp.json()
    assert "mini_flow" in data
