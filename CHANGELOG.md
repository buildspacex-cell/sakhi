...
### Changed
- **Web `/pitch` now supports fullscreen presentation mode** — the fixed deck navigation now includes a fullscreen toggle so the investor pitch can enter and exit browser fullscreen during live presentation
- **Web `/story` and `/pitch` moments panel now uses the continuity visual** — the first-expression product walkthrough in `apps/web/components/story/Scene5Email.tsx` and `apps/web/components/pitch/PitchProduct.tsx` now swaps the old moments timeline image for the continuity screen so both presentation surfaces stay aligned
- **Web `/story` playback controls work again across all scenes** — the cinematic runtime now keeps the pause/restart/scrub controls above every scene layer, uses live range-input scrubbing while dragging, and drives GSAP playback with explicit play/pause/seek calls so the timeline remains controllable after recent scene and narration changes
- **Web `/story` and `/pitch` product walkthrough now removes the synthesis panel** — the first-expression product sequence in `apps/web/components/story/Scene5Email.tsx` and `apps/web/components/pitch/PitchProduct.tsx` now runs conversation, reflection, moments, and privacy only, and the cinematic timeline was shortened accordingly
- **Web `/story` and `/pitch` product walkthrough now removes the grounded-reflection panel** — the first-expression product sequence in `apps/web/components/story/Scene5Email.tsx` and `apps/web/components/pitch/PitchProduct.tsx` now runs conversation, reflection, moments, synthesis, and privacy without the separate grounded-reflection beat, and the cinematic timeline was retimed accordingly
- **Web `/story` and `/pitch` now pause on a founder bridge beat** — both presentation surfaces now insert a dedicated transition slide, `On the people who are building this...`, before the founder chapters begin so the narrative cleanly pivots from product/vision into the founding story
- **Web `/pitch` now mirrors the cinematic story deck** — the pitch route now follows the same narrative order as the latest `/story` video runtime (`problem` → `Sakhi reveal` → `continuity` → `product` → `vision` → `founders` → `close`), activates the continuity section, adds the Sakhi voice beat, and replaces the generic GTM/roadmap flow with the founder-story deck structure
- **Web `/story` narration unlock is now reliable on first play** — the story speech hook now buffers early narration until the browser speech engine is fully primed from a user gesture, preventing the first spoken lines from being dropped on the cinematic route
- **Web `/story` final scene now uses a shared founder contact CTA** — the close scene in `apps/web/components/story/Scene8Close.tsx` now keeps separate collaboration and investment blurbs while routing both intents through one shared founder email link for outreach
...
- **Continuity thread-depth model** — continuity compilation now distinguishes strict primary moments, thread-attached follow-up moments, and projected linked-topic overlap; chat Deep Reflect uses effective same-thread depth (`primary + attached`) so obvious follow-up journal entries can deepen a topic without repeating the anchor name
- **Mobile MVP UI foundation** — active iOS MVP screens now share a centralized theme token layer and reusable screen/button/card/icon/loading primitives, while Settings persists compact-mode and haptics preferences and core chat/reflection/support flows use the same premium interaction language
- **Web MVP proportion rebalance** — the local Midnight Editorial web pass now uses a tighter typography scale and calmer screen spacing so chat, auth, home, reflection, settings, and support feel balanced at normal browser zoom without shrinking controls
...
### Fixed
- **Mobile sign-out state reset** - mobile sign-out now clears the cached linked `person_id`, performs a local Supabase session logout, and lets the root auth gate route protected screens back to `/auth`, preventing half-signed-out states where profile-bound screens could linger on stale identity data
- **Mobile linked-profile hydration after auth bootstrap** - Google/email sign-in bootstrap now writes the resolved `person_id` through to in-memory mobile auth state (not just cache/route params), so Settings, Reflection, and other profile-bound surfaces stop showing `Not linked yet` immediately after a successful login
...
- **Mobile tactile/loading polish** - chat, reflection, settings, support, home, and auth now honor a real persisted haptics toggle, use shared press feedback on key actions, and show skeleton-style loading blocks for history/topic loading without changing conversation or reflection logic
- **Mobile Midnight Editorial refresh** - active iOS MVP surfaces now use the cooler smoked-indigo editorial palette from web, with decorative orb backgrounds removed from chat/account surfaces while reflection keeps the actual life-occupancy bubble language
...
- **Journal continuity identity consistency** — active journal write paths now populate both `journal_entries.person_id` and `journal_entries.user_id`, and a backfill migration restores legacy rows that only wrote `user_id`, preventing continuity/debug surfaces from depending on partially-populated journal identity columns
