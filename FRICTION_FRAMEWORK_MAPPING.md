...
Sakhi already implements **~60-65% of the Friction Framework** through its sophisticated Ayurvedic intelligence layer. The system computes doshas, gunas, elements, and soul states—but the critical Knowledge Graph reasoning engine is NOT yet built (only schema exists).

**The Hidden Engine Strategy:** Ayurveda powers everything, users see "Operating Systems" and "Friction States."

**CRITICAL GAP:** The Knowledge Graph (Ayurvedic reasoning engine connecting doshas → foods → habits → outcomes) exists only as database schema. Implementation of this reasoning layer is essential for intelligent, personalized recommendations that differentiate Sakhi from basic journaling apps.
